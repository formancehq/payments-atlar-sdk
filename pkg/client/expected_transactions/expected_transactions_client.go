...
func (a *Client) PostV1betaExpectedTransactionsIDUnarchive(params *PostV1betaExpectedTransactionsIDUnarchiveParams, opts ...ClientOption) (*PostV1betaExpectedTransactionsIDUnarchiveOK, error) {
	// TODO: Validate the params before sending
	if params == nil {
		params = NewPostV1betaExpectedTransactionsIDUnarchiveParams()
	}
	op := &runtime.ClientOperation{
		ID:                 "PostV1betaExpectedTransactionsIDUnarchive",
		Method:             "POST",
		PathPattern:        "/v1beta/expected-transactions/{id}:unarchive",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"https"},
		Params:             params,
		Reader:             &PostV1betaExpectedTransactionsIDUnarchiveReader{formats: a.formats},
		Context:            params.Context,
		Client:             params.HTTPClient,
	}
	for _, opt := range opts {
	...
	success, ok := result.(*PostV1betaExpectedTransactionsIDUnarchiveOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for PostV1betaExpectedTransactionsIDUnarchive: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
