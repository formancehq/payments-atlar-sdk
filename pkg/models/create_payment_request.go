...
type CreatePaymentRequest struct {
	...
	// The payment scheme that is desired. Choose among SCT (SEPA Credit Transfer), SCT_INST (SEPA Instant Credit Transfer), GB_CT_BACS (UK account-to-account BACS), GB_CT_CHAPS (UK account-to-account CHAPS), GB_CT_FPS (UK account-to-account Faster Payments), SE_A2A (Swedish account-to-account), SE_GIRO (Swedish Bank/Plus-giro), DK_A2A (Danish account-to-account), DK_CT_SAMEDAY (Danish same-day credit transfer), DK_CT_INST (Danish instant credit transfer), DK_CT_FIK71 (Danish FIK71 credit transfer), DK_CT_FIK73 (Danish FIK73 credit transfer), DK_CT_FIK75 (Danish FIK75 credit transfer), NO_CT (Norwegian credit transfer), US_CT_ACH (US ACH credit transfer), US_CT_Wire (US Wire credit transfer), AU_CT (Australian credit transfer), AU_CT_BPAY (Australian BPAY credit transfer), IL_CT (Israel credit transfer), SG_CT (Singapore credit transfer), BG_CT (Bulgarian credit transfer), MX_CT (Mexican credit transfer), KR_CT (South Korean credit transfer), TH_CT (Thai credit transfer), CROSS_BORDER (swift).
	// Example: SCT
	// Required: true
	// Enum: [SCT SCT_INST GB_CT_BACS GB_CT_CHAPS GB_CT_FPS SE_A2A SE_GIRO DK_A2A DK_CT_SAMEDAY DK_CT_INST DK_CT_FIK71 DK_CT_FIK73 DK_CT_FIK75 CH_CT NO_CT US_CT_ACH US_CT_WIRE AU_CT AU_CT_BPAY SG_CT BG_CT MX_CT KR_CT TH_CT CROSS_BORDER]
...
func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["SCT","SCT_INST","GB_CT_BACS","GB_CT_CHAPS","GB_CT_FPS","SE_A2A","SE_GIRO","DK_A2A","DK_CT_SAMEDAY","DK_CT_INST","DK_CT_FIK71","DK_CT_FIK73","DK_CT_FIK75","CH_CT","NO_CT","US_CT_ACH","US_CT_WIRE","AU_CT","AU_CT_BPAY","SG_CT","BG_CT","MX_CT","KR_CT","TH_CT","CROSS_BORDER"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
...
const (
	...
	CreatePaymentRequestPaymentSchemeTypeGBCTCHAPS string = "GB_CT_CHAPS"
	...
	CreatePaymentRequestPaymentSchemeTypeDKCTFIK71 string = "DK_CT_FIK71"

	// CreatePaymentRequestPaymentSchemeTypeDKCTFIK73 captures enum value "DK_CT_FIK73"
	CreatePaymentRequestPaymentSchemeTypeDKCTFIK73 string = "DK_CT_FIK73"

	// CreatePaymentRequestPaymentSchemeTypeDKCTFIK75 captures enum value "DK_CT_FIK75"
	CreatePaymentRequestPaymentSchemeTypeDKCTFIK75 string = "DK_CT_FIK75"
